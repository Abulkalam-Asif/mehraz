          ...
          {expanded && (
            <div
              ...
              {options.map(({ label, value }, index) => (
                <div
                  key={index}
                  className="flex items-center border-b border-b-accent-2-base">
                  <input
                    id={`option${index}`}
                    type="checkbox"
                    value={value}
                    ...
                    className="text-accent-1-extra-dark rounded-md peer w-0 h-0 focus:outline-none"
                  />
                  <label
                    className="text-sm cursor-pointer peer-checked:font-bold p-2 hover:bg-accent-1-base"
